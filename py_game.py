...
class ShootingManager:
    ...
    def wounded(self, x, y):
        ...
        for i in [(x + 1, y + 1), (x - 1, y - 1), (x + 1, y - 1),
                  (x - 1, y + 1)]:
            self.player.cells_state[i] = False
    ...
    def killed(self, x, y):
        ...
        for neighbour in neighbours:
            self.drawer.put_cross(cell_size * (neighbour[0] - 1 + self.__offset + offset_for_field) +
                                  ...
                                  top_margin, RED)
            self.player.cells_state[neighbour] = False
        dots = []
        ...
        for dot in dots:
            self.player.cells_state[dot] = False
...
class DrawManager:
    ...
    def draw_field_settings_window(self, field_params):
        ...
        for i in range(4, 0, -1):
            for j in range(i + 1):
                pygame.draw.line(screen, BLACK, (x_start + j * cell_size, y_start),
            ...
            if event.type == pygame.QUIT:
                ...
                if drawer.next_btn.rect.collidepoint(mouse):
                    if are_params_correct():
                        field_set_up = True
                        field_params = FieldParams(field_params.field_size, field_params.num_4, field_params.num_3,
                                                   field_params.num_2, field_params.num_1)
